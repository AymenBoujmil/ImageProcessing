...
def filterApply(imageMatrix, ndG, nbLines, nbCols,n,filterName,sigma = 1,number = 1):
  if filterName == "Moynenneur":
    filter = filtreMoyenne(n)
    return convolution(imageMatrix, ndG, nbLines, nbCols,filter,n)
  if filterName == "Gaussian":
    filter = filterGaussian(n,sigma)
    return convolution(imageMatrix, ndG, nbLines, nbCols,filter,n)
  if filterName == "Median":
    return filterMedian(imageMatrix, ndG, nbLines, nbCols,n)
  if filterName == "passeHaut":
    filter = filtrePasseHaut(number)
    return convolution(imageMatrix, ndG, nbLines, nbCols,filter,n)
